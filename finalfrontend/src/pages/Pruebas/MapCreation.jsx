...
export const MapCreation = () => {
  ...
  useEffect(() => {
    const createMap = () => {
      if (province && postcode && postcodes[province] && postcodes[province][postcode]) {
    ...
    <>
      <div className='background'>
        <select name="province" id="province" onChange={(e) => setProvince(e.target.value)}>
          {Object.entries(postcodes).sort((a,b)=> a - b).map(([province]) => (
            <option key={province} value={province}>{province}</option>
          ))}
        </select>
        {province && postcodes[province] && (
          <select name="postcodes" id="postcode" onChange={(e) => setPostcode(e.target.value)}>
            {Object.entries(postcodes[province]).sort((a, b) => a - b).map(([postcode]) => (
